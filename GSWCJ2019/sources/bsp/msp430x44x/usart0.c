 ...
 * @retval 0 成功，其他失败
 */
uint8_t USART0Init(void)
{
  return USART0SetParam(4800, 0);
}
 ...
 * @retval 0 成功，其他失败
 */
uint8_t USART0SetParam(uint32_t baud, uint8_t parity)
{
  uint16_t br_div = 0;
  uint32_t br_clk = 0;
 ...
 * @retval 实际发送数据长度
 */
uint16_t USART0Send(const uint8_t *pbuf, uint16_t len)
{
	uint16_t tmout = 0;
	uint16_t len_tmp = 0;
 ...
 * @retval 0无数据包,1有数据包
 */
uint8_t USART0CheckReceive(void)
{
  uint8_t ret = 0;
 ...
 * @retval 接收数据长度
 */
uint16_t USART0GetReceiveLength(void)
{
 ...
 * @retval 串口接收最后字符时间
 */
uint32_t USART0GetLastRecvCharTick(void)
{
